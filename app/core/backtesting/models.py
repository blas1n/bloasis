...
Pure data models with no infrastructure dependencies.
...
class BacktestConfig(BaseModel):
    ...
    initial_cash: Decimal = Field(
        ...
        description="Initial capital for simulation (USD)",
    )
    commission: float = Field(
        default=0.001,
        ge=0,
        le=0.1,
        description="Commission per trade (e.g., 0.001 = 0.1%)",
    )
    slippage: float = Field(
        default=0.001,
        ge=0,
        le=0.1,
        description="Slippage per trade (e.g., 0.001 = 0.1%)",
    )
    stop_loss: float = Field(
        default=0.0,
        ge=0,
        le=1,
        description="Stop loss threshold (0 = disabled)",
    )
    take_profit: float = Field(
        default=0.0,
        ge=0,
        le=10,
        description="Take profit threshold (0 = disabled)",
    )
...
class SymbolResult(BaseModel):
    ...
    passed: bool = Field(
        default=False,
        description="Whether strategy passes minimum criteria",
    )
...
class PortfolioMetrics(BaseModel):
    ...
    total_return: float = Field(
        default=0.0,
        description="Average total return across all symbols",
    )
    sharpe_ratio: float = Field(default=0.0, description="Average Sharpe ratio")
    max_drawdown: float = Field(
        default=0.0,
        description="Maximum drawdown (worst across symbols)",
    )
...
class BacktestResult(BaseModel):
    ...
    results: list[SymbolResult] = Field(
        ...
        description="Per-symbol results",
    )
